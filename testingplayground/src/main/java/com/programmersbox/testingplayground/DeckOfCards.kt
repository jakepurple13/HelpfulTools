...
class Deck<T> : AbstractDeck<T> {
    ...
    constructor(vararg cards: T, listener: (DeckListenerBuilder<T>.() -> Unit)?) : super(*cards) {
        listener?.let(this::addDeckListener)
    }

    constructor(cards: Iterable<T>, listener: (DeckListenerBuilder<T>.() -> Unit)?) : super(cards) {
        listener?.let(this::addDeckListener)
    }
