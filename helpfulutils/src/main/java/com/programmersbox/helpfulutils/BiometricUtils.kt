...
class BiometricBuilder(private var fragmentActivity: FragmentActivity) {
    ...
    enum class BiometricErrorType(@Suppress("unused") var num: Int, var reason: String) {
    ...
    private var biometricPromptInfo: BiometricPrompt.PromptInfo by Delegates.notNull()
    ...
    private var executors: Executor = Executors.newSingleThreadExecutor()

    @BiometricMarker
    fun setExecutor(executor: Executor) = run { executors = executor }
    ...
    private fun build() {
        when (BiometricManager.from(fragmentActivity).canAuthenticate()) {
            BiometricManager.BIOMETRIC_SUCCESS -> BiometricPrompt(fragmentActivity, executors,
